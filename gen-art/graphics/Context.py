...
from graphics.Helpers import does_path_exist, open_file
from os import path
from datetime import datetime


class DrawContext:
    def __init__(self, width, height, file_name, file_format, filepath, project_folder, open_bool):
    ...
    def set_cwd(self):
        if self.file_format == "PNG":
            return path.dirname(path.realpath(__file__))[:-9] + self.filepath
        elif self.file_format == "SVG":
            return path.dirname(path.realpath(__file__))[:-9] + self.filepath + "/0-svg"
        else:
